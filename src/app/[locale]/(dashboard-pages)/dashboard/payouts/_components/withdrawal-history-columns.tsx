...
export const useWithdrawalHistoryColumns = (): TableColumnDefinition<WithdrawalTransaction>[] => {
  ...
  return [
    ...
    },
    {
      ...
      accessorKey: "amount",
      render: (_, transaction: WithdrawalTransaction) => (
        <span className="font-semibold text-gray-900">{formatCurrency(transaction.amount, locale as Locale)}</span>
      ),
    },
    {
      header: "Bank",
      ...
      render: (_, transaction: WithdrawalTransaction) => (
        <div>
          <div className="font-medium text-gray-900">{transaction.bankName}</div>
          <div className="text-sm text-gray-500">{transaction.bankAccount}</div>
        </div>
      ),
    },
    {
      ...
      accessorKey: "date",
      render: (_, transaction: WithdrawalTransaction) => (
        <span className="text-gray-700">{formatDate(transaction.date, locale as Locale)}</span>
      ),
    },
    {
      header: "Status",
      ...
      render: (_, transaction: WithdrawalTransaction) => (
        <span
          className={cn(
            ...
            transaction.status === "success" && "bg-low-success text-mid-success",

...
const WithdrawalDetailsPage = ({ params }: PageProperties) => {
  ...
  const { data: withdrawalResponse, isLoading, isError, refetch } = useGetWithdrawalDetails(params.id);
  ...
  if (isLoading) {
    return (
      <div className="min-h-[calc(100vh-130px)] space-y-8">
        {/* Header Skeleton */}
  ...
  if (isError || !withdrawalData) {
    return (
      <div className="min-h-[calc(100vh-130px)] space-y-8">
        <DashboardHeader title="Withdraw Details" showSubscriptionBanner={false} icon={<BackButton />} />
        <ErrorState className={`!bg-background`} onRetry={() => refetch()} />
      </div>
    );
  }

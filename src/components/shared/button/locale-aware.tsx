...
/**
 * LocaleAwareButton component that automatically handles locale for internal links
 * This is an enhanced version of SkiButton that preserves locale when navigating
 */
const LocaleAwareButton = forwardRef<HTMLButtonElement, ButtonProperties>(
  (
    {
      type = "button",
      variant,
    ...
    if (href) {
      ...
      // For internal links, automatically add locale if not present
      const localizedHref = localizeHref(href, locale);

      return (
        <Link href={isDisabled ? "" : localizedHref} passHref aria-label={ariaLabel}>
          <Button
            variant={variant}
            size={size}
            disabled={isDisabled}
...
LocaleAwareButton.displayName = "LocaleAwareButton";

export default LocaleAwareButton;
